...
class NTU_Reader():
    def __init__(self, args, root_folder, transform, ntu60_path, ntu120_path, **kwargs):
        ...
        ntu_actions = '{}/actions.txt'.format(os.path.dirname(os.path.realpath(__file__)))
        if self.transform:
        ...
        # Get actions list
        try:
            self.actions = np.loadtxt(ntu_actions, dtype=np.int)
        except:
            self.actions = np.arange(1,61)
    ...
    def gendata(self, phase):
        ...
        for folder, filename in iterizer:
            if filename in self.ignored_samples:
                continue
            elif int(filename[filename.find('A') + 1:filename.find('A') + 4]) not in self.actions:
                continue
            ...
            action_class = np.where(self.actions == int(filename[filename.find('A') + 1:filename.find('A') + 4]))[0][0]
